...
export async function getPassList(req: Request, res: Response): Promise<void> {
    try {
        ...
        // }
        const dataCheckins: any = groupingCheckinsByPerformanceAndWhere(dataByPerformance);

        // レスポンス編集
        const data: any = {schedules: []};
        Object.keys(dataByPerformance).forEach((performanceId) => {
            ...
            if (Object.keys(checkins).length > 0) {
                ...
                Object.keys(checkins).forEach((where) => {
                    checkpoint.id = where;
                    checkpoint.name = (checkpointNames.hasOwnProperty(where)) ? (<any>checkpointNames)[where] : where;
    ...
    reservations.map(async(reservation: any) => {
        ...
        if (isExtra) {
            ...
            if (!reservedExtra.hasOwnProperty(reservation.ticket_type)) {
                ...
                (<any>reservedExtra).reservedNum += 1;
            }
        } else {
 ...
 * @returns {Promise<any>}
 */
function groupingCheckinsByPerformanceAndWhere(dataByPerformance: any): any {
...
function getConcernedUnarrivedArray(concernedReservedArray: any[], checkins: any[]): any {
