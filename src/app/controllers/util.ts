...
export async function performancestatus(req: Request, res: Response): Promise<void> {
    ...
    try {
        ...
        const token = await getToken(currentToken, [`${<string>process.env.API_RESOURECE_SERVER_IDENTIFIER}/performances.read-only`]);
        if (token.error) {
        ...
        const rawRes = await fetch(`${process.env.API_ENDPOINT}performances?${reqGetParams}`, {
            headers: {
 ...
 * @returns {Promise<void>}
 */
async function getToken (currentToken: any, scopes: string[]): Promise<any> {
    if (currentToken.expirationDateISO && moment().isBefore(moment(currentToken.expirationDateISO))) {
        return currentToken;
    }
    let token: any = {};
    try {
        token = <any>await ttts.CommonUtil.getToken({
